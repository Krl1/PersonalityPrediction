...
class MLP(pl.LightningModule):
    ...
    def __init__(
        ...
        while fm_size != 8:
            ...
            if self.dropout != 0.0 and fm_size!=16:
                self.linear.add_module(f"dropout_{i}", nn.Dropout(self.dropout))
            fm_size = fm_size//2
            i+=1

        if self.dropout != 0.0:
            self.linear.add_module(f"dropout_{i}", nn.Dropout(self.dropout/2))
...
class BiggerMLP(pl.LightningModule):
    ...
    def __init__(
        ...
        while fm_size != 8:
            if b==1:
                self.linear.add_module(f"linear_{i}", nn.Linear(fm_size, fm_size))
                if self.batch_norm:
                    self.linear.add_module(f"batch_norm_{i}", nn.BatchNorm1d(fm_size))
                b=0
            else:
                self.linear.add_module(f"linear_{i}", nn.Linear(fm_size, fm_size//2))
                if self.batch_norm:
            ...
            if b==1:
                fm_size = fm_size//2
            if self.dropout != 0.0 and fm_size!=8:
                self.linear.add_module(f"dropout_{i}", nn.Dropout(self.dropout))
            i+=1
